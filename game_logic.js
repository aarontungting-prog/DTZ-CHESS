...
<div id="loading">正在載入霓虹世界...</div>

<div id="mobile-menu-btn" onclick="toggleMenu()">☰ 選單</div>
...
<div id="custom-panel" class="side-panel">
    ...
    <div class="custom-section">
        ...
        <div class="skin-grid" id="piece-skin-grid">
            <div class="skin-item selected" onclick="previewSkin('piece', 'neon', this)">
                ...
                <div class="skin-name">電馭霓虹</div>
            </div>
            <div class="skin-item" onclick="previewSkin('piece', 'classic', this)">
                <div class="skin-icon">♟️</div>
                <div class="skin-name">經典實體</div>
            </div>
            <div class="skin-item locked">
                <div class="skin-icon">🔒</div>
                <div class="skin-name">敬請期待</div>
            </div>
        </div>
    ...
    <button onclick="closeAllMenus()" class="game-btn" style="background:#333; margin-top:10px;">關閉面板</button>
</div>
...
<script type="module">
    import { initGame, triggerAvatarUpload, previewStyle } from './game_logic.js';
    window.triggerAvatarUpload = triggerAvatarUpload; 

    window.previewSkin = function(type, value, element) {
        if(element.classList.contains('locked')) return;
        const grid = document.getElementById(`${type}-skin-grid`);
        const items = grid.getElementsByClassName('skin-item');
        for(let item of items) item.classList.remove('selected');
        element.classList.add('selected');
        document.getElementById(`selected-${type}-style`).value = value;
        previewStyle(type, value);
    }
